...
This file records why r2 is structured this way.
...
r2 therefore does not mutate that backend in place. It adds a separate
`n64libdragon` platform backend and leaves the old backend untouched as
reference material.

## libdragon baseline

r2 targets libdragon stable `trunk` but pins the tested source snapshot to commit `35f85a0797324a5ed0c723203e33ab3c1da94fdd` (2026-07-15).
...
libdragon's Newlib integration provides the C/POSIX file layer. r2 therefore
...
* The current libdragon build defaults to C++17; r2 filters that default and builds the 2013 ScummVM code as GNU++11 to reduce avoidable language-version breakage.
