...
# Do not gate the build by parsing `make -n` command text. Previous revisions
# proved that this is less reliable than the real build. Actual prior builds compiled through
# the full SCUMM tree to the linker and linked through to n64tool. The
# authoritative verification is now the actual V=1 build log plus output ROM.
# Keep the structural Make database checks above, then let compilation/linking
# and ROM packaging report their own exact commands and exit status.
