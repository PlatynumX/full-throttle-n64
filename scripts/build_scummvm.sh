...
ART="$ROOT/artifacts"
ROM="full-throttle-n64-r2v-v20.z64"
mkdir -p "$ART"
...
ELF="$PORT/build/full-throttle-n64-r2v-v20.elf"

if [ "$rc" -eq 0 ] && [ -f "$PORT/$ROM" ] && [ -f "$ELF" ]; then
    cp "$PORT/$ROM" "$ART/$ROM"
    cp "$ELF" "$ART/full-throttle-n64-r2v-v20.elf"
    sha256sum "$ART/$ROM" > "$ART/full-throttle-n64-r2v-v20.sha256"
    sha256sum "$ART/full-throttle-n64-r2v-v20.elf" > "$ART/full-throttle-n64-r2v-v20-elf.sha256"
    ...
    MAP="$PORT/build/full-throttle-n64-r2v-v20.map"
    if [ -f "$MAP" ]; then
        cp "$MAP" "$ART/full-throttle-n64-r2v-v20.map"
    fi
