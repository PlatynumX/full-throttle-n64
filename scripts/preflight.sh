...
echo "[preflight] required r2i backend gates"
...
if grep -R -nE '^N64_ROM_CONTROLLER1[[:space:]]*[:?+]?=[[:space:]]*joypad$' backend probe; then
  echo "invalid libdragon ROM-header controller metadata leaked into r2i" >&2
  exit 1
fi
...
if grep -R -nE '<dirent\.h>|opendir\(|readdir\(|closedir\(|backends/fs/posix' backend probe; then
  echo "unsupported POSIX directory dependency leaked into r2i" >&2
  exit 1
fi
if grep -R -nE 'mkdir\("sd:/' backend probe; then
  echo "runtime SD mkdir leaked into r2i; pinned libdragon FAT has no mkdir hook" >&2
  exit 1
fi
...
if grep -R -nE 'hkz-libn64|libn64\.h|pakfs|framfs|initRomFSmanager|NONSTANDARD_PORT' backend probe; then
  echo "legacy N64 dependency leaked into r2i backend" >&2
  exit 1
fi
...
if grep -nE 'run: \./scripts/|^[[:space:]]+\./scripts/' .github/workflows/build-full-throttle-r2i.yml; then
...
grep -q 'id: probe' .github/workflows/build-full-throttle-r2i.yml
grep -q 'id: scummvm' .github/workflows/build-full-throttle-r2i.yml
[ "$(grep -c 'continue-on-error: true' .github/workflows/build-full-throttle-r2i.yml)" -eq 2 ]
...
if grep -RInE 'r2[a-g]|R2[A-G]' .github backend probe scripts demo upstream TERMUX.md; then
  echo "stale prior-revision operational label leaked into r2i" >&2
  exit 1
fi
...
grep -q 'toolchain-continuous-prerelease/gcc-toolchain-mips64-x86_64.deb' .github/workflows/build-full-throttle-r2i.yml
...
grep -Fq 'if [ "$rc" -eq 0 ] && [ -f "$PORT/full-throttle-n64-r2i.z64" ]; then' scripts/build_scummvm.sh

echo "[preflight] generated-make flag ownership audit"
grep -Fq 'make -C "$DST" -n full-throttle-n64-r2i.z64' scripts/integrate_backend.sh
...
grep -Fq 'capture && /-Wl,-Map=build\/full-throttle-n64-r2i\.map;/ { exit }' scripts/integrate_backend.sh
if grep -Fq 'link_line="$(grep' scripts/integrate_backend.sh; then
  echo "obsolete first-physical-line linker audit leaked into r2i" >&2
  exit 1
fi
