...
DEST="${HOME}/ft64-r2u-push"

printf '[publish] validating pristine r2u tree\n'
...
if git diff --cached --quiet; then
    echo '[publish] remote already matches r2u; nothing to commit.'
    exit 0
fi

git commit -m 'Full Throttle N64 r2u memory-map diagnostics'
...
if [ "$remote_head" != "$parent_head" ]; then
    echo "remote master moved after this r2u tree was prepared" >&2
...
echo '[publish] r2u pushed successfully.'
