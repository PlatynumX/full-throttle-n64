...
DEST="${HOME}/ft64-r2f-push"

printf '[publish] validating pristine r2f tree\n'
...
if git diff --cached --quiet; then
  echo '[publish] remote already matches r2f; nothing to commit.'
  exit 0
fi

git commit -m 'Full Throttle N64 r2f clean backend baseline'
# Race-safe without ever force pushing. Actions has contents:read and cannot move master.
git fetch origin master
git rebase origin/master
git push origin master

echo '[publish] r2f pushed successfully.'
