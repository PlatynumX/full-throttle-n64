...
DEST="${HOME}/ft64-r2p-push"

printf '[publish] validating pristine r2p tree\n'
...
if git diff --cached --quiet; then
  echo '[publish] remote already matches r2p; nothing to commit.'
  exit 0
fi

git commit -m 'Full Throttle N64 r2p SMUSH timing and overlay transition fixes'
git fetch origin master
git rebase origin/master
git push origin master

echo '[publish] r2p pushed successfully.'
