...
DEST="${HOME}/ft64-r2v-push"

printf '[publish] validating pristine r2v tree\n'
...
if git diff --cached --quiet; then
    echo '[publish] remote already matches r2v; nothing to commit.'
    exit 0
fi

git commit -m 'Full Throttle N64 r2v Full Throttle-only pruning'
...
if [ "$remote_head" != "$parent_head" ]; then
    echo "remote master moved after this r2v tree was prepared" >&2
...
echo '[publish] r2v pushed successfully.'
