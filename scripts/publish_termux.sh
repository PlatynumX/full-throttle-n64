...
DEST="${HOME}/ft64-r2q-push"

printf '[publish] validating pristine r2q tree\n'
...
if git diff --cached --quiet; then
    echo '[publish] remote already matches r2q; nothing to commit.'
    exit 0
fi

git commit -m 'Full Throttle N64 r2q exact-source SMUSH patch'
...
if [ "$remote_head" != "$parent_head" ]; then
    echo "remote master moved after this r2q tree was prepared" >&2
...
echo '[publish] r2q pushed successfully.'
