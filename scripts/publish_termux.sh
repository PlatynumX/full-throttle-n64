...
DEST="${HOME}/ft64-r2s-push"

printf '[publish] validating pristine r2s tree\n'
...
if git diff --cached --quiet; then
    echo '[publish] remote already matches r2s; nothing to commit.'
    exit 0
fi

git commit -m 'Full Throttle N64 r2s retail-transition diagnostics'
...
if [ "$remote_head" != "$parent_head" ]; then
    echo "remote master moved after this r2s tree was prepared" >&2
...
echo '[publish] r2s pushed successfully.'
