...
DEST="${HOME}/ft64-r2r-push"

printf '[publish] validating pristine r2r tree\n'
...
if git diff --cached --quiet; then
    echo '[publish] remote already matches r2r; nothing to commit.'
    exit 0
fi

git commit -m 'Full Throttle N64 r2r retail-transition diagnostics'
...
if [ "$remote_head" != "$parent_head" ]; then
    echo "remote master moved after this r2r tree was prepared" >&2
...
echo '[publish] r2r pushed successfully.'
