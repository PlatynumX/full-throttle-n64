...
DEST="${HOME}/ft64-r2t-push"

printf '[publish] validating pristine r2t tree\n'
...
if git diff --cached --quiet; then
    echo '[publish] remote already matches r2t; nothing to commit.'
    exit 0
fi

git commit -m 'Full Throttle N64 r2t retail-transition diagnostics'
...
if [ "$remote_head" != "$parent_head" ]; then
    echo "remote master moved after this r2t tree was prepared" >&2
...
echo '[publish] r2t pushed successfully.'
