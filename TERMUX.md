# Full Throttle N64 r2j — Termux

Keep the extracted project in Termux home, not Android shared storage.

```bash
cd ~
rm -rf ~/full-throttle-n64-r2j
unzip -o ~/storage/downloads/full-throttle-n64-r2j.zip -d ~
cd ~/full-throttle-n64-r2j
...
1. validates the extracted r2j tree;
...
The push triggers `build-full-throttle-r2j.yml` automatically.

To manually start or watch the workflow:

```bash
gh workflow run build-full-throttle-r2j.yml
gh run watch
```
