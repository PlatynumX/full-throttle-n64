# Full Throttle N64 r2i — Termux

Keep the extracted project in Termux home, not Android shared storage.

```bash
cd ~
rm -rf ~/full-throttle-n64-r2i
unzip -o ~/storage/downloads/full-throttle-n64-r2i.zip -d ~
cd ~/full-throttle-n64-r2i
...
1. validates the extracted r2i tree;
...
The push triggers `build-full-throttle-r2i.yml` automatically.

To manually start or watch the workflow:

```bash
gh workflow run build-full-throttle-r2i.yml
gh run watch
```
