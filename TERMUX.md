# Full Throttle N64 r2k — Termux

Keep the extracted project in Termux home, not Android shared storage.

```bash
cd ~
rm -rf ~/full-throttle-n64-r2k
unzip -o ~/storage/downloads/full-throttle-n64-r2k.zip -d ~
cd ~/full-throttle-n64-r2k
...
1. validates the extracted r2k tree;
...
The push triggers `build-full-throttle-r2k.yml` automatically.

To manually start or watch the workflow:

```bash
gh workflow run build-full-throttle-r2k.yml
gh run watch
```
