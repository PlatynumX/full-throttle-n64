# Full Throttle N64 r2h — Termux

Keep the extracted project in Termux home, not Android shared storage.

```bash
cd ~
rm -rf ~/full-throttle-n64-r2h
unzip -o ~/storage/downloads/full-throttle-n64-r2h.zip -d ~
cd ~/full-throttle-n64-r2h
...
1. validates the extracted r2h tree;
...
The push triggers `build-full-throttle-r2h.yml` automatically.

To manually start or watch the workflow:

```bash
gh workflow run build-full-throttle-r2h.yml
gh run watch
```
