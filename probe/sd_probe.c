...
static bool run_rw_test(void) {
    static const char payload[] = "Full Throttle N64 r2f SD probe OK\n";
    FILE *f = fopen("sd:/fullthrottle/ft64-r2f-probe.txt", "wb");
    if (!f)
        return false;
    ...
    f = fopen("sd:/fullthrottle/ft64-r2f-probe.txt", "rb");
    if (!f)
        return false;
...
int main(void) {
    ...
    debugf("FT64 r2f probe: expanded=%d mem=%lu sd=%d demo=%d saves=%d rw=%d audio=%dHz\n",
    ...
    while (1) {
        ...
        if (pressed.a) {
            FILE *f = fopen("sd:/fullthrottle/ft64-r2f-probe.txt", "ab");
            if (f) {
                fputs("A button append OK\n", f);
                fclose(f);
                debugf("FT64 r2f: A-button SD append OK\n");
            } else {
                debugf("FT64 r2f: A-button SD append FAILED errno=%d\n", errno);
            }
        }
