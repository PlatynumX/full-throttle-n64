...
static bool run_rw_test(void) {
    static const char payload[] = "Full Throttle N64 r2t SD probe OK\n";
    FILE *f = fopen("sd:/fullthrottle/ft64-r2t-probe.txt", "wb");
    if (!f)
        return false;
    ...
    f = fopen("sd:/fullthrottle/ft64-r2t-probe.txt", "rb");
    if (!f)
        return false;
...
int main(void) {
    ...
    lines[15] = "";
    draw_lines(lines, 16);
    debugf("FT64 r2t probe: expanded=%d mem=%lu sd=%d game=%d saves=%d rw=%d audio=%dHz\n",
    ...
    while (1) {
        ...
        if (pressed.a) {
            FILE *f = fopen("sd:/fullthrottle/ft64-r2t-probe.txt", "ab");
            if (f) {
                fputs("A button append OK\n", f);
                fclose(f);
                debugf("FT64 r2t: A-button SD append OK\n");
            } else {
                debugf("FT64 r2t: A-button SD append FAILED errno=%d\n", errno);
            }
        }
